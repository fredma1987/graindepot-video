...
public interface HCNetSDK extends StdCallLibrary {
    ...
    int MAX_ETHERNET = 2;   //设备可配以太网络
    int PATHNAME_LEN = 128;   //路径长度
    int MAX_TIMESEGMENT_V30 = 8;    //9000设备最大时间段数
    int MAX_TIMESEGMENT = 4;   //8000设备最大时间段数
    int MAX_SHELTERNUM = 4;   //8000设备最大遮挡区域数
    int MAX_DAYS = 7;      //每周天数
    int PHONENUMBER_LEN = 32;   //pppoe拨号号码最大长度
    int MAX_DISKNUM_V30 = 33;		//9000设备最大硬盘数/* 最多33个硬盘(包括16个内置SATA硬盘、1个eSATA硬盘和16个NFS盘) */
    int MAX_DISKNUM = 16;     //8000设备最大硬盘数
    int MAX_DISKNUM_V10 = 8;   //1.2版本之前版本
    int MAX_WINDOW_V30 = 32; //9000设备本地显示最大播放窗口数
    int MAX_WINDOW = 16;    //8000设备最大硬盘数
    int MAX_VGA_V30 = 4;     //9000设备最大可接VGA数
    int MAX_VGA = 1;    //8000设备最大可接VGA数
    int MAX_USERNUM_V30 = 32;  //9000设备最大用户数
    int MAX_USERNUM = 16;  //8000设备最大用户数
    int MAX_EXCEPTIONNUM_V30 = 32;  //9000设备最大异常处理数
    int MAX_EXCEPTIONNUM = 16;   //8000设备最大异常处理数
    int MAX_LINK = 6;    //8000设备单通道最大视频流连接数
    int MAX_DECPOOLNUM = 4;   //单路解码器每个解码通道最大可循环解码数
    int MAX_DECNUM = 4;    //单路解码器的最大解码通道数（实际只有一个，其他三个保留）
    int MAX_TRANSPARENTNUM = 2;   //单路解码器可配置最大透明通道数
    int MAX_CYCLE_CHAN = 16;   //单路解码器最大轮循通道数
    int MAX_DIRNAME_LENGTH = 80;   //最大目录长度
    int MAX_STRINGNUM_V30 = 8;		//9000设备最大OSD字符行数数
    int MAX_STRINGNUM = 4;   //8000设备最大OSD字符行数数
    int MAX_STRINGNUM_EX = 8;   //8000定制扩展
    int MAX_AUXOUT_V30 = 16;   //9000设备最大辅助输出数
    int MAX_AUXOUT = 4;      //8000设备最大辅助输出数
    int MAX_HD_GROUP = 16;   //9000设备最大硬盘组数
    int MAX_NFS_DISK = 8;    //8000设备最大NFS硬盘数
    int IW_ESSID_MAX_SIZE = 32;    //WIFI的SSID号长度
    int IW_ENCODING_TOKEN_MAX = 32;   //WIFI密锁最大字节数
    int MAX_SERIAL_NUM = 64;    //最多支持的透明通道路数
    int MAX_DDNS_NUMS = 10;   //9000设备最大可配ddns数
    int MAX_DOMAIN_NAME = 64;	/* 最大域名长度 */

    int MAX_EMAIL_ADDR_LEN = 48;  //最大email地址长度
    int MAX_EMAIL_PWD_LEN = 32;     //最大email密码长度
    int MAXPROGRESS = 100;  //回放时的最大百分率
    int MAX_SERIALNUM = 2;    //8000设备支持的串口数 1-232， 2-485
    int CARDNUM_LEN = 20;    //卡号长度
    int MAX_VIDEOOUT_V30 = 4;      //9000设备的视频输出数
    int MAX_VIDEOOUT = 2;      //8000设备的视频输出数
    int MAX_PRESET_V30 = 256;	/* 9000设备支持的云台预置点数 */
    int MAX_TRACK_V30 = 256;	/* 9000设备支持的云台轨迹数 */
    int MAX_CRUISE_V30 = 256;	/* 9000设备支持的云台巡航数 */
    int MAX_PRESET = 128;	/* 8000设备支持的云台预置点数 */
    int MAX_TRACK = 128;	/* 8000设备支持的云台轨迹数 */
    int MAX_CRUISE = 128;	/* 8000设备支持的云台巡航数 */
    int CRUISE_MAX_PRESET_NUMS = 32;    /* 一条巡航最多的巡航点 */
    int MAX_SERIAL_PORT = 8;    //9000设备支持232串口数
    int MAX_PREVIEW_MODE = 8;    /* 设备支持最大预览模式数目 1画面,4画面,9画面,16画面.... */
    int MAX_MATRIXOUT = 16;  /* 最大模拟矩阵输出个数 */
    int LOG_INFO_LEN = 11840; /* 日志附加信息 */
    int DESC_LEN = 16;    /* 云台描述字符串长度 */
    int PTZ_PROTOCOL_NUM = 200;   /* 9000最大支持的云台协议数 */
    int MAX_AUDIO = 1;    //8000语音对讲通道数
    int MAX_AUDIO_V30 = 2;   //9000语音对讲通道数
    int MAX_CHANNUM = 16;   //8000设备最大通道数
    int MAX_ALARMIN = 16;  //8000设备最大报警输入数
    int MAX_ALARMOUT = 4;    //8000设备最大报警输出数
//9000 IPC接入
int MAX_ANALOG_CHANNUM = 32;    //最大32个模拟通道
    int MAX_ANALOG_ALARMOUT = 32;    //最大32路模拟报警输出
    int MAX_ANALOG_ALARMIN = 32;    //最大32路模拟报警输入
    int MAX_IP_DEVICE = 32;    //允许接入的最大IP设备数
    int MAX_IP_CHANNEL = 32;   //允许加入的最多IP通道数
    int MAX_IP_ALARMIN = 128;   //允许加入的最多报警输入数
    int MAX_IP_ALARMOUT = 64; //允许加入的最多报警输出数

    /* 最大支持的通道数 最大模拟加上最大IP支持 */
    int MAX_CHANNUM_V30 = (MAX_ANALOG_CHANNUM + MAX_IP_CHANNEL);//64
    int MAX_ALARMOUT_V30 = (MAX_ANALOG_ALARMOUT + MAX_IP_ALARMOUT);//96
    int MAX_ALARMIN_V30 = (MAX_ANALOG_ALARMIN + MAX_IP_ALARMIN);//160

    int MAX_LICENSE_LEN = 32;
    int VCA_MAX_POLYGON_POINT_NUM = 10;
    /*******************全局错误码 begin**********************/
    int NET_DVR_NOERROR = 0;	//没有错误
    int NET_DVR_PASSWORD_ERROR = 1;	//用户名密码错误
    int NET_DVR_NOENOUGHPRI = 2;//权限不足
    int NET_DVR_NOINIT = 3;//没有初始化
    int NET_DVR_CHANNEL_ERROR = 4;	//通道号错误
    int NET_DVR_OVER_MAXLINK = 5;	//连接到DVR的客户端个数超过最大
    int NET_DVR_VERSIONNOMATCH = 6;	//版本不匹配
    int NET_DVR_NETWORK_FAIL_CONNECT = 7;//连接服务器失败
    int NET_DVR_NETWORK_SEND_ERROR = 8;	//向服务器发送失败
    int NET_DVR_NETWORK_RECV_ERROR = 9;	//从服务器接收数据失败
    int NET_DVR_NETWORK_RECV_TIMEOUT = 10;	//从服务器接收数据超时
    int NET_DVR_NETWORK_ERRORDATA = 11;	//传送的数据有误
    int NET_DVR_ORDER_ERROR = 12;	//调用次序错误
    int NET_DVR_OPERNOPERMIT = 13;	//无此权限
    int NET_DVR_COMMANDTIMEOUT = 14;	//DVR命令执行超时
    int NET_DVR_ERRORSERIALPORT = 15;	//串口号错误
    int NET_DVR_ERRORALARMPORT = 16;	//报警端口错误
    int NET_DVR_PARAMETER_ERROR = 17;//参数错误
    int NET_DVR_CHAN_EXCEPTION = 18;	//服务器通道处于错误状态
    int NET_DVR_NODISK = 19;	//没有硬盘
    int NET_DVR_ERRORDISKNUM = 20;	//硬盘号错误
    int NET_DVR_DISK_FULL = 21;	//服务器硬盘满
    int NET_DVR_DISK_ERROR = 22;//服务器硬盘出错
    int NET_DVR_NOSUPPORT = 23;//服务器不支持
    int NET_DVR_BUSY = 24;//服务器忙
    int NET_DVR_MODIFY_FAIL = 25;//服务器修改不成功
    int NET_DVR_PASSWORD_FORMAT_ERROR = 26;//密码输入格式不正确
    int NET_DVR_DISK_FORMATING = 27;	//硬盘正在格式化，不能启动操作
    int NET_DVR_DVRNORESOURCE = 28;	//DVR资源不足
    int NET_DVR_DVROPRATEFAILED = 29; //DVR操作失败
    int NET_DVR_OPENHOSTSOUND_FAIL = 30; //打开PC声音失败
    int NET_DVR_DVRVOICEOPENED = 31; //服务器语音对讲被占用
    int NET_DVR_TIMEINPUTERROR = 32; //时间输入不正确
    int NET_DVR_NOSPECFILE = 33;  //回放时服务器没有指定的文件
    int NET_DVR_CREATEFILE_ERROR = 34;	//创建文件出错
    int NET_DVR_FILEOPENFAIL = 35; //打开文件出错
    int NET_DVR_OPERNOTFINISH = 36; //上次的操作还没有完成
    int NET_DVR_GETPLAYTIMEFAIL = 37; //获取当前播放的时间出错
    int NET_DVR_PLAYFAIL = 38; //播放出错
    int NET_DVR_FILEFORMAT_ERROR = 39;//文件格式不正确
    int NET_DVR_DIR_ERROR = 40;	//路径错误
    int NET_DVR_ALLOC_RESOURCE_ERROR = 41;//资源分配错误
    int NET_DVR_AUDIO_MODE_ERROR = 42;	//声卡模式错误
    int NET_DVR_NOENOUGH_BUF = 43;	//缓冲区太小
    int NET_DVR_CREATESOCKET_ERROR = 44;	//创建SOCKET出错
    int NET_DVR_SETSOCKET_ERROR = 45;	//设置SOCKET出错
    int NET_DVR_MAX_NUM = 46;	//个数达到最大
    int NET_DVR_USERNOTEXIST = 47;	//用户不存在
    int NET_DVR_WRITEFLASHERROR = 48;//写FLASH出错
    int NET_DVR_UPGRADEFAIL = 49;//DVR升级失败
    int NET_DVR_CARDHAVEINIT = 50; //解码卡已经初始化过
    int NET_DVR_PLAYERFAILED = 51;	//调用播放库中某个函数失败
    int NET_DVR_MAX_USERNUM = 52; //设备端用户数达到最大
    int NET_DVR_GETLOCALIPANDMACFAIL = 53;//获得客户端的IP地址或物理地址失败
    int NET_DVR_NOENCODEING = 54;	//该通道没有编码
    int NET_DVR_IPMISMATCH = 55;	//IP地址不匹配
    int NET_DVR_MACMISMATCH = 56;//MAC地址不匹配
    int NET_DVR_UPGRADELANGMISMATCH = 57;//升级文件语言不匹配
    int NET_DVR_MAX_PLAYERPORT = 58;//播放器路数达到最大
    int NET_DVR_NOSPACEBACKUP = 59;//备份设备中没有足够空间进行备份
    int NET_DVR_NODEVICEBACKUP = 60;	//没有找到指定的备份设备
    int NET_DVR_PICTURE_BITS_ERROR = 61;	//图像素位数不符，限24色
    int NET_DVR_PICTURE_DIMENSION_ERROR = 62;//图片高*宽超限， 限128*256
    int NET_DVR_PICTURE_SIZ_ERROR = 63;	//图片大小超限，限100K
    int NET_DVR_LOADPLAYERSDKFAILED = 64;	//载入当前目录下Player Sdk出错
    int NET_DVR_LOADPLAYERSDKPROC_ERROR = 65;	//找不到Player Sdk中某个函数入口
    int NET_DVR_LOADDSSDKFAILED = 66;	//载入当前目录下DSsdk出错
    int NET_DVR_LOADDSSDKPROC_ERROR = 67;	//找不到DsSdk中某个函数入口
    int NET_DVR_DSSDK_ERROR = 68;	//调用硬解码库DsSdk中某个函数失败
    int NET_DVR_VOICEMONOPOLIZE = 69;	//声卡被独占
    int NET_DVR_JOINMULTICASTFAILED = 70;	//加入多播组失败
    int NET_DVR_CREATEDIR_ERROR = 71;	//建立日志文件目录失败
    int NET_DVR_BINDSOCKET_ERROR = 72;	//绑定套接字失败
    int NET_DVR_SOCKETCLOSE_ERROR = 73;	//socket连接中断，此错误通常是由于连接中断或目的地不可达
    int NET_DVR_USERID_ISUSING = 74;	//注销时用户ID正在进行某操作
    int NET_DVR_SOCKETLISTEN_ERROR = 75;	//监听失败
    int NET_DVR_PROGRAM_EXCEPTION = 76;	//程序异常
    int NET_DVR_WRITEFILE_FAILED = 77;	//写文件失败
    int NET_DVR_FORMAT_READONLY = 78;//禁止格式化只读硬盘
    int NET_DVR_WITHSAMEUSERNAME = 79;//用户配置结构中存在相同的用户名
    int NET_DVR_DEVICETYPE_ERROR = 80; /*导入参数时设备型号不匹配*/
    int NET_DVR_LANGUAGE_ERROR = 81; /*导入参数时语言不匹配*/
    int NET_DVR_PARAVERSION_ERROR = 82; /*导入参数时软件版本不匹配*/
    int NET_DVR_IPCHAN_NOTALIVE = 83; /*预览时外接IP通道不在线*/
    int NET_DVR_RTSP_SDK_ERROR = 84;	/*加载高清IPC通讯库StreamTransClient.dll失败*/
    int NET_DVR_CONVERT_SDK_ERROR = 85;	/*加载转码库失败*/
    int NET_DVR_IPC_COUNT_OVERFLOW = 86; /*超出最大的ip接入通道数*/
    int NET_PLAYM4_NOERROR = 500;	//no error
    int NET_PLAYM4_PARA_OVER = 501;//input parameter is invalid;
    int NET_PLAYM4_ORDER_ERROR = 502;//The order of the function to be called is error.
    int NET_PLAYM4_TIMER_ERROR = 503;//Create multimedia clock failed;
    int NET_PLAYM4_DEC_VIDEO_ERROR = 504;//Decode video data failed.
    int NET_PLAYM4_DEC_AUDIO_ERROR = 505;//Decode audio data failed.
    int NET_PLAYM4_ALLOC_MEMORY_ERROR = 506;	//Allocate memory failed.
    int NET_PLAYM4_OPEN_FILE_ERROR = 507;	//Open the file failed.
    int NET_PLAYM4_CREATE_OBJ_ERROR = 508;//Create thread or event failed
    int NET_PLAYM4_CREATE_DDRAW_ERROR = 509;//Create DirectDraw object failed.
    int NET_PLAYM4_CREATE_OFFSCREEN_ERROR = 510;//failed when creating off-screen surface.
    int NET_PLAYM4_BUF_OVER = 511;	//buffer is overflow
    int NET_PLAYM4_CREATE_SOUND_ERROR = 512;	//failed when creating audio device.
    int NET_PLAYM4_SET_VOLUME_ERROR = 513;//Set volume failed
    int NET_PLAYM4_SUPPORT_FILE_ONLY = 514;//The function only support play file.
    int NET_PLAYM4_SUPPORT_STREAM_ONLY = 515;//The function only support play stream.
    int NET_PLAYM4_SYS_NOT_SUPPORT = 516;//System not support.
    int NET_PLAYM4_FILEHEADER_UNKNOWN = 517;	//No file header.
    int NET_PLAYM4_VERSION_INCORRECT = 518;	//The version of decoder and encoder is not adapted.
    int NET_PALYM4_INIT_DECODER_ERROR = 519;	//Initialize decoder failed.
    int NET_PLAYM4_CHECK_FILE_ERROR = 520;	//The file data is unknown.
    int NET_PLAYM4_INIT_TIMER_ERROR = 521;	//Initialize multimedia clock failed.
    int NET_PLAYM4_BLT_ERROR = 522;//Blt failed.
    int NET_PLAYM4_UPDATE_ERROR = 523;//Update failed.
    int NET_PLAYM4_OPEN_FILE_ERROR_MULTI = 524; //openfile error, streamtype is multi
    int NET_PLAYM4_OPEN_FILE_ERROR_VIDEO = 525; //openfile error, streamtype is video
    int NET_PLAYM4_JPEG_COMPRESS_ERROR = 526; //JPEG compress error
    int NET_PLAYM4_EXTRACT_NOT_SUPPORT = 527;	//Don't support the version of this file.
    int NET_PLAYM4_EXTRACT_DATA_ERROR = 528;	//extract video data failed.
    /*******************全局错误码 end**********************/
    /*************************************************
    NET_DVR_IsSupport()返回值
    1－9位分别表示以下信息（位与是TRUE)表示支持；
     **************************************************/
    int NET_DVR_SUPPORT_DDRAW = 0x01;//支持DIRECTDRAW，如果不支持，则播放器不能工作；
    int NET_DVR_SUPPORT_BLT = 0x02;//显卡支持BLT操作，如果不支持，则播放器不能工作；
    int NET_DVR_SUPPORT_BLTFOURCC = 0x04;//显卡BLT支持颜色转换，如果不支持，播放器会用软件方法作RGB转换；
    int NET_DVR_SUPPORT_BLTSHRINKX = 0x08;//显卡BLT支持X轴缩小；如果不支持，系统会用软件方法转换；
    int NET_DVR_SUPPORT_BLTSHRINKY = 0x10;//显卡BLT支持Y轴缩小；如果不支持，系统会用软件方法转换；
    int NET_DVR_SUPPORT_BLTSTRETCHX = 0x20;//显卡BLT支持X轴放大；如果不支持，系统会用软件方法转换；
    int NET_DVR_SUPPORT_BLTSTRETCHY = 0x40;//显卡BLT支持Y轴放大；如果不支持，系统会用软件方法转换；
    int NET_DVR_SUPPORT_SSE = 0x80;//CPU支持SSE指令，Intel Pentium3以上支持SSE指令；
    int NET_DVR_SUPPORT_MMX = 0x100;//CPU支持MMX指令集，Intel Pentium3以上支持SSE指令；
    /**********************云台控制命令 begin*************************/
    int LIGHT_PWRON = 2;	/* 接通灯光电源 */
    int WIPER_PWRON = 3;	/* 接通雨刷开关 */
    int FAN_PWRON = 4;	/* 接通风扇开关 */
    int HEATER_PWRON = 5;	/* 接通加热器开关 */
    int AUX_PWRON1 = 6;	/* 接通辅助设备开关 */
    int AUX_PWRON2 = 7;	/* 接通辅助设备开关 */
    ...
    int FILL_PRE_SEQ = 30;	/* 将预置点加入巡航序列 */
    int SET_SEQ_DWELL = 31;	/* 设置巡航点停顿时间 */
    int SET_SEQ_SPEED = 32;	/* 设置巡航速度 */
    int CLE_PRE_SEQ = 33;/* 将预置点从巡航序列中删除 */
    int STA_MEM_CRUISE = 34;/* 开始记录轨迹 */
    int STO_MEM_CRUISE = 35;/* 停止记录轨迹 */
    int RUN_CRUISE = 36;	/* 开始轨迹 */
    int RUN_SEQ = 37;	/* 开始巡航 */
    int STOP_SEQ = 38;	/* 停止巡航 */
    ...
    int NET_DVR_PLAYSTOP = 2;//停止播放
    int NET_DVR_PLAYPAUSE = 3;//暂停播放
    int NET_DVR_PLAYRESTART = 4;//恢复播放
    int NET_DVR_PLAYFAST = 5;//快放
    int NET_DVR_PLAYSLOW = 6;//慢放
    int NET_DVR_PLAYNORMAL = 7;//正常速度
    int NET_DVR_PLAYFRAME = 8;//单帧放
    int NET_DVR_PLAYSTARTAUDIO = 9;//打开声音
    int NET_DVR_PLAYSTOPAUDIO = 10;//关闭声音
    int NET_DVR_PLAYAUDIOVOLUME = 11;//调节音量
    int NET_DVR_PLAYSETPOS = 12;//改变文件回放的进度
    int NET_DVR_PLAYGETPOS = 13;//获取文件回放的进度
    int NET_DVR_PLAYGETTIME = 14;//获取当前已经播放的时间(按文件回放的时候有效)
    int NET_DVR_PLAYGETFRAME = 15;//获取当前已经播放的帧数(按文件回放的时候有效)
    int NET_DVR_GETTOTALFRAMES = 16;//获取当前播放文件总的帧数(按文件回放的时候有效)
    int NET_DVR_GETTOTALTIME = 17;//获取当前播放文件总的时间(按文件回放的时候有效)
    int NET_DVR_THROWBFRAME = 20;//丢B帧
    int NET_DVR_SETSPEED = 24;//设置码流速度
    int NET_DVR_KEEPALIVE = 25;//保持与设备的心跳(如果回调阻塞，建议2秒发送一次)
//远程按键定义如下：
...
int KEY_CODE_1 = 1;
    ...
    /* for PTZ control */
    int KEY_PTZ_UP_START = KEY_CODE_UP;
    int KEY_PTZ_UP_STO = 32;
    int KEY_PTZ_DOWN_START = KEY_CODE_DOWN;
    int KEY_PTZ_DOWN_STOP = 33;
    int KEY_PTZ_LEFT_START = KEY_CODE_LEFT;
    int KEY_PTZ_LEFT_STOP = 34;
    int KEY_PTZ_RIGHT_START = KEY_CODE_RIGHT;
    int KEY_PTZ_RIGHT_STOP = 35;
    int KEY_PTZ_AP1_START = KEY_CODE_EDIT;/* 光圈+ */
    int KEY_PTZ_AP1_STOP = 36;
    int KEY_PTZ_AP2_START = KEY_CODE_PAN;/* 光圈- */
    int KEY_PTZ_AP2_STOP = 37;
    int KEY_PTZ_FOCUS1_START = KEY_CODE_A;/* 聚焦+ */
    int KEY_PTZ_FOCUS1_STOP = 38;
    int KEY_PTZ_FOCUS2_START = KEY_CODE_M ;/* 聚焦- */
    int KEY_PTZ_FOCUS2_STOP = 39;
    int KEY_PTZ_B1_START = 40;/* 变倍+ */
    int KEY_PTZ_B1_STOP = 41;
    int KEY_PTZ_B2_START = 42;/* 变倍- */
...
//用于NET_DVR_SetDVRConfig和NET_DVR_GetDVRConfig,注意其对应的配置结构
    int NET_DVR_GET_DEVICECFG = 100;	//获取设备参数
    int NET_DVR_SET_DEVICECFG = 101;	//设置设备参数
    int NET_DVR_GET_NETCFG = 102;	//获取网络参数
    int NET_DVR_SET_NETCFG = 103;	//设置网络参数
    int NET_DVR_GET_PICCFG = 104;	//获取图象参数
    int NET_DVR_SET_PICCFG = 105;	//设置图象参数
    int NET_DVR_GET_COMPRESSCFG = 106;	//获取压缩参数
    int NET_DVR_SET_COMPRESSCFG = 107;	//设置压缩参数
    int NET_DVR_GET_RECORDCFG = 108;	//获取录像时间参数
    int NET_DVR_SET_RECORDCFG = 109;	//设置录像时间参数
    int NET_DVR_GET_DECODERCFG = 110;	//获取解码器参数
    int NET_DVR_SET_DECODERCFG = 111;	//设置解码器参数
    int NET_DVR_GET_RS232CFG = 112;	//获取232串口参数
    int NET_DVR_SET_RS232CFG = 113;	//设置232串口参数
    int NET_DVR_GET_ALARMINCFG = 114;	//获取报警输入参数
    int NET_DVR_SET_ALARMINCFG = 115;	//设置报警输入参数
    int NET_DVR_GET_ALARMOUTCFG = 116;	//获取报警输出参数
    int NET_DVR_SET_ALARMOUTCFG = 117;	//设置报警输出参数
    int NET_DVR_GET_TIMECFG = 118;	//获取DVR时间
    int NET_DVR_SET_TIMECFG = 119;		//设置DVR时间
    int NET_DVR_GET_PREVIEWCFG = 120;	//获取预览参数
    int NET_DVR_SET_PREVIEWCFG = 121;	//设置预览参数
    int NET_DVR_GET_VIDEOOUTCFG = 122;	//获取视频输出参数
    int NET_DVR_SET_VIDEOOUTCFG = 123;	//设置视频输出参数
    int NET_DVR_GET_USERCFG = 124;	//获取用户参数
    int NET_DVR_SET_USERCFG = 125;	//设置用户参数
    int NET_DVR_GET_EXCEPTIONCFG = 126;	//获取异常参数
    int NET_DVR_SET_EXCEPTIONCFG = 127;	//设置异常参数
    int NET_DVR_GET_ZONEANDDST = 128;	//获取时区和夏时制参数
    int NET_DVR_SET_ZONEANDDST = 129;	//设置时区和夏时制参数
    int NET_DVR_GET_SHOWSTRING = 130;	//获取叠加字符参数
    int NET_DVR_SET_SHOWSTRING = 131;	//设置叠加字符参数
    int NET_DVR_GET_EVENTCOMPCFG = 132;	//获取事件触发录像参数
    int NET_DVR_SET_EVENTCOMPCFG = 133;	//设置事件触发录像参数
    int NET_DVR_GET_AUXOUTCFG = 140;	//获取报警触发辅助输出设置(HS设备辅助输出2006-02-28)
    int NET_DVR_SET_AUXOUTCFG = 141;	//设置报警触发辅助输出设置(HS设备辅助输出2006-02-28)
    int NET_DVR_GET_PREVIEWCFG_AUX = 142;	//获取-s系列双输出预览参数(-s系列双输出2006-04-13)
    int NET_DVR_SET_PREVIEWCFG_AUX = 143;	//设置-s系列双输出预览参数(-s系列双输出2006-04-13)
    int NET_DVR_GET_PICCFG_EX = 200;	//获取图象参数(SDK_V14扩展命令)
    int NET_DVR_SET_PICCFG_EX = 201;	//设置图象参数(SDK_V14扩展命令)
    int NET_DVR_GET_USERCFG_EX = 202;	//获取用户参数(SDK_V15扩展命令)
    int NET_DVR_SET_USERCFG_EX = 203;	//设置用户参数(SDK_V15扩展命令)
    int NET_DVR_GET_COMPRESSCFG_EX = 204;	//获取压缩参数(SDK_V15扩展命令2006-05-15)
    int NET_DVR_SET_COMPRESSCFG_EX = 205;	//设置压缩参数(SDK_V15扩展命令2006-05-15)
    int NET_DVR_GET_NETAPPCFG = 222;	//获取网络应用参数 NTP/DDNS/EMAIL
    int NET_DVR_SET_NETAPPCFG = 223;	//设置网络应用参数 NTP/DDNS/EMAIL
    int NET_DVR_GET_NTPCFG = 224;	//获取网络应用参数 NTP
    int NET_DVR_SET_NTPCFG = 225;	//设置网络应用参数 NTP
    int NET_DVR_GET_DDNSCFG = 226;	//获取网络应用参数 DDNS
    int NET_DVR_SET_DDNSCFG = 227;		//设置网络应用参数 DDNS
//对应NET_DVR_EMAILPARA
int NET_DVR_GET_EMAILCFG = 228;	//获取网络应用参数 EMAIL
    int NET_DVR_SET_EMAILCFG = 229;	//设置网络应用参数 EMAIL
    int NET_DVR_GET_NFSCFG = 230;	/* NFS disk config */
    int NET_DVR_SET_NFSCFG = 231;	/* NFS disk config */
    int NET_DVR_GET_SHOWSTRING_EX = 238;	//获取叠加字符参数扩展(支持8条字符)
    int NET_DVR_SET_SHOWSTRING_EX = 239;	//设置叠加字符参数扩展(支持8条字符)
    int NET_DVR_GET_NETCFG_OTHER = 244;	//获取网络参数
    int NET_DVR_SET_NETCFG_OTHER = 245;	//设置网络参数
//对应NET_DVR_EMAILCFG结构
int NET_DVR_GET_EMAILPARACFG = 250;	//Get EMAIL parameters
    int NET_DVR_SET_EMAILPARACFG = 251;	//Setup EMAIL parameters
    int NET_DVR_GET_DDNSCFG_EX = 274;//获取扩展DDNS参数
    int NET_DVR_SET_DDNSCFG_EX = 275;//设置扩展DDNS参数
    int NET_DVR_SET_PTZPOS = 292;	//云台设置PTZ位置
    int NET_DVR_GET_PTZPOS = 293;		//云台获取PTZ位置
    int NET_DVR_GET_PTZSCOPE = 294;//云台获取PTZ范围
    /***************************DS9000新增命令(_V30) begin *****************************/
//网络(NET_DVR_NETCFG_V30结构)
    int NET_DVR_GET_NETCFG_V30 = 1000;	//获取网络参数
    int NET_DVR_SET_NETCFG_V30 = 1001;	//设置网络参数
//图象(NET_DVR_PICCFG_V30结构)
int NET_DVR_GET_PICCFG_V30 = 1002;	//获取图象参数
    int NET_DVR_SET_PICCFG_V30 = 1003;	//设置图象参数
//录像时间(NET_DVR_RECORD_V30结构)
int NET_DVR_GET_RECORDCFG_V30 = 1004;	//获取录像参数
    int NET_DVR_SET_RECORDCFG_V30 = 1005;	//设置录像参数
//用户(NET_DVR_USER_V30结构)
int NET_DVR_GET_USERCFG_V30 = 1006;	//获取用户参数
    int NET_DVR_SET_USERCFG_V30 = 1007;	//设置用户参数
//9000DDNS参数配置(NET_DVR_DDNSPARA_V30结构)
int NET_DVR_GET_DDNSCFG_V30 = 1010;	//获取DDNS(9000扩展)
    int NET_DVR_SET_DDNSCFG_V30 = 1011;	//设置DDNS(9000扩展)
//EMAIL功能(NET_DVR_EMAILCFG_V30结构)
int NET_DVR_GET_EMAILCFG_V30 = 1012;//获取EMAIL参数
    int NET_DVR_SET_EMAILCFG_V30 = 1013;//设置EMAIL参数
...
int NET_DVR_GET_DECODERCFG_V30 = 1042;	//获取解码器参数
    int NET_DVR_SET_DECODERCFG_V30 = 1043;	//设置解码器参数
//获取预览参数 (NET_DVR_PREVIEWCFG_V30结构)
int NET_DVR_GET_PREVIEWCFG_V30 = 1044;	//获取预览参数
    int NET_DVR_SET_PREVIEWCFG_V30 = 1045;	//设置预览参数
//辅助预览参数 (NET_DVR_PREVIEWCFG_AUX_V30结构)
int NET_DVR_GET_PREVIEWCFG_AUX_V30 = 1046;	//获取辅助预览参数
    int NET_DVR_SET_PREVIEWCFG_AUX_V30 = 1047;	//设置辅助预览参数
...
int NET_DVR_GET_IPALARMINCFG = 1050;    //获取IP报警输入接入配置信息
    int NET_DVR_SET_IPALARMINCFG = 1051;   //设置IP报警输入接入配置信息
//IP报警输出接入配置参数 （NET_DVR_IPALARMOUTCFG结构）
int NET_DVR_GET_IPALARMOUTCFG = 1052;   //获取IP报警输出接入配置信息
    int NET_DVR_SET_IPALARMOUTCFG = 1053;  //设置IP报警输出接入配置信息
//硬盘管理的参数获取 (NET_DVR_HDCFG结构)
int NET_DVR_GET_HDCFG = 1054;    //获取硬盘管理配置参数
    int NET_DVR_SET_HDCFG = 1055;    //设置硬盘管理配置参数
//盘组管理的参数获取 (NET_DVR_HDGROUP_CFG结构)
int NET_DVR_GET_HDGROUP_CFG = 1056;    //获取盘组管理配置参数
    int NET_DVR_SET_HDGROUP_CFG = 1057;    //设置盘组管理配置参数
//设备编码类型配置(NET_DVR_COMPRESSION_AUDIO结构)
int NET_DVR_GET_COMPRESSCFG_AUD = 1058;     //获取设备语音对讲编码参数
    int NET_DVR_SET_COMPRESSCFG_AUD = 1059;     //设置设备语音对讲编码参数
    /***************************DS9000新增命令(_V30) end *****************************/
    /*************************参数配置命令 end*******************************/
    /*******************查找文件和日志函数返回值*************************/
    int NET_DVR_FILE_SUCCESS = 1000;	//获得文件信息
    int NET_DVR_FILE_NOFIND = 1001;	//没有文件
    int NET_DVR_ISFINDING = 1002;//正在查找文件
    int NET_DVR_NOMOREFILE = 1003;//查找文件时没有更多的文件
    int NET_DVR_FILE_EXCEPTION = 1004;//查找文件时异常
    /*********************回调函数类型 begin************************/
    ...
    int COMM_SNAP_MATCH_ALARM = 0x2902;  //黑名单比对结果上传
    /*************操作异常类型(消息方式, 回调方式(保留))****************/
    int EXCEPTION_EXCHANGE = 0x8000;//用户交互时异常
    int EXCEPTION_AUDIOEXCHANGE = 0x8001;//语音对讲异常
    int EXCEPTION_ALARM = 0x8002;//报警异常
    int EXCEPTION_PREVIEW = 0x8003;//网络预览异常
    int EXCEPTION_SERIAL = 0x8004;//透明通道异常
    int EXCEPTION_RECONNECT = 0x8005;	//预览时重连
    int EXCEPTION_ALARMRECONNECT = 0x8006;//报警时重连
    int EXCEPTION_SERIALRECONNECT = 0x8007;//透明通道重连
    int EXCEPTION_PLAYBACK = 0x8010;//回放异常
    int EXCEPTION_DISKFMT = 0x8011;//硬盘格式化
    /********************预览回调函数*********************/
...
//回调预览中的状态和消息
int NET_DVR_REALPLAYEXCEPTION = 111;//预览异常
    int NET_DVR_REALPLAYNETCLOSE = 112;//预览时连接断开
    int NET_DVR_REALPLAY5SNODATA = 113;//预览5s没有收到数据
    int NET_DVR_REALPLAYRECONNECT = 114;//预览重连
    /********************回放回调函数*********************/
    int NET_DVR_PLAYBACKOVER = 101;//回放数据播放完毕
    int NET_DVR_PLAYBACKEXCEPTION = 102;//回放异常
    int NET_DVR_PLAYBACKNETCLOSE = 103;//回放时候连接断开
    int NET_DVR_PLAYBACK5SNODATA = 104;	//回放5s没有收到数据
    /*********************回调函数类型 end************************/
//设备型号(DVR类型)
/* 设备类型 */
    int DVR = 1;			/*对尚未定义的dvr类型返回NETRET_DVR*/
    int ATMDVR = 2;		/*atm dvr*/
    int DVS = 3;			/*DVS*/
    int DEC = 4;			/* 6001D */
    int ENC_DEC = 5;			/* 6001F */
    int DVR_HC = 6;			/*8000HC*/
    int DVR_HT = 7;			/*8000HT*/
    int DVR_HF = 8;			/*8000HF*/
    int DVR_HS = 9;			/* 8000HS DVR(no audio) */
    int DVR_HTS = 10;         /* 8016HTS DVR(no audio) */
    int DVR_HB = 11;         /* HB DVR(SATA HD) */
    int DVR_HCS = 12;         /* 8000HCS DVR */
    int DVS_A = 13;         /* 带ATA硬盘的DVS */
    int DVR_HC_S = 14;         /* 8000HC-S */
    int DVR_HT_S = 15;         /* 8000HT-S */
    int DVR_HF_S = 16;         /* 8000HF-S */
    int DVR_HS_S = 17;         /* 8000HS-S */
    int ATMDVR_S = 18;         /* ATM-S */
    int LOWCOST_DVR = 19;			/*7000H系列*/
    int DEC_MAT = 20;         /*多路解码器*/
    int DVR_MOBILE = 21;			/* mobile DVR */
    int DVR_HD_S = 22;        /* 8000HD-S */
    int DVR_HD_SL = 23;			/* 8000HD-SL */
    int DVR_HC_SL = 24;			/* 8000HC-SL */
    int DVR_HS_ST = 25;			/* 8000HS_ST */
    int DVS_HW = 26;         /* 6000HW */
    int IPCAM = 30;			/*IP 摄像机*/
    int MEGA_IPCAM = 31;			/*X52MF系列,752MF,852MF*/
    int IPCAM_X62MF = 32;			/*X62MF系列可接入9000设备,762MF,862MF*/
    int IPDOME = 40;			/*IP标清快球*/
    int MEGA_IPDOME = 41;     /*IP高清快球*/
    int IPMOD = 50;			/*IP 模块*/
    int DS71XX_H = 71;			/* DS71XXH_S */
    int DS72XX_H_S = 72;			/* DS72XXH_S */
    int DS73XX_H_S = 73;			/* DS73XXH_S */
    int DS81XX_HS_S = 81;			/* DS81XX_HS_S */
    int DS81XX_HL_S = 82;			/* DS81XX_HL_S */
    int DS81XX_HC_S = 83;			/* DS81XX_HC_S */
    int DS81XX_HD_S = 84;			/* DS81XX_HD_S */
    int DS81XX_HE_S = 85;			/* DS81XX_HE_S */
    int DS81XX_HF_S = 86;			/* DS81XX_HF_S */
    int DS81XX_AH_S = 87;			/* DS81XX_AH_S */
    int DS81XX_AHF_S = 88;			/* DS81XX_AHF_S */
    int DS90XX_HF_S = 90;       /*DS90XX_HF_S*/
    int DS91XX_HF_S = 91;             /*DS91XX_HF_S*/
    int DS91XX_HD_S = 92;            /*91XXHD-S(MD)*/

    /* 操作 */
//主类型
    int MAJOR_OPERATION = 0x3;
//次类型
int MINOR_START_DVR = 0x41; /* 开机 */
    int MINOR_STOP_DVR = 0x42;/* 关机 */
    int MINOR_STOP_ABNORMAL = 0x43;/* 异常关机 */
    int MINOR_REBOOT_DVR = 0x44;   /*本地重启设备*/
    int MINOR_LOCAL_LOGIN = 0x50; /* 本地登陆 */
    int MINOR_LOCAL_LOGOUT = 0x51; /* 本地注销登陆 */
    int MINOR_LOCAL_CFG_PARM = 0x52; /* 本地配置参数 */
    int MINOR_LOCAL_PLAYBYFILE = 0x53; /* 本地按文件回放或下载 */
    int MINOR_LOCAL_PLAYBYTIME = 0x54; /* 本地按时间回放或下载*/
    int MINOR_LOCAL_START_REC = 0x55; /* 本地开始录像 */
    int MINOR_LOCAL_STOP_REC = 0x56; /* 本地停止录像 */
    int MINOR_LOCAL_PTZCTRL = 0x57; /* 本地云台控制 */
    int MINOR_LOCAL_PREVIEW = 0x58;/* 本地预览 (保留不使用)*/
    int MINOR_LOCAL_MODIFY_TIME = 0x59;/* 本地修改时间(保留不使用) */
    int MINOR_LOCAL_UPGRADE = 0x5a;/* 本地升级 */
    int MINOR_LOCAL_RECFILE_OUTPUT = 0x5b;   /* 本地备份录象文件 */
    int MINOR_LOCAL_FORMAT_HDD = 0x5c;  /* 本地初始化硬盘 */
    int MINOR_LOCAL_CFGFILE_OUTPUT = 0x5d;  /* 导出本地配置文件 */
    int MINOR_LOCAL_CFGFILE_INPUT = 0x5e;  /* 导入本地配置文件 */
    int MINOR_LOCAL_COPYFILE = 0x5f;  /* 本地备份文件 */
    int MINOR_LOCAL_LOCKFILE = 0x60;  /* 本地锁定录像文件 */
    int MINOR_LOCAL_UNLOCKFILE = 0x61;   /* 本地解锁录像文件 */
    int MINOR_LOCAL_DVR_ALARM = 0x62;  /* 本地手动清除和触发报警*/
    int MINOR_IPC_ADD = 0x63;  /* 本地添加IPC */
    int MINOR_IPC_DEL = 0x64;  /* 本地删除IPC */
    int MINOR_IPC_SET = 0x65;  /* 本地设置IPC */
    int MINOR_LOCAL_START_BACKUP = 0x66;	/* 本地开始备份 */
    int MINOR_LOCAL_STOP_BACKUP = 0x67;/* 本地停止备份*/
    int MINOR_LOCAL_COPYFILE_START_TIME = 0x68;/* 本地备份开始时间*/
    int MINOR_LOCAL_COPYFILE_END_TIME = 0x69;	/* 本地备份结束时间*/
    int MINOR_REMOTE_LOGIN = 0x70;/* 远程登录 */
    int MINOR_REMOTE_LOGOUT = 0x71;/* 远程注销登陆 */
    int MINOR_REMOTE_START_REC = 0x72;/* 远程开始录像 */
    int MINOR_REMOTE_STOP_REC = 0x73;/* 远程停止录像 */
    int MINOR_START_TRANS_CHAN = 0x74;/* 开始透明传输 */
    int MINOR_STOP_TRANS_CHAN = 0x75; /* 停止透明传输 */
    int MINOR_REMOTE_GET_PARM = 0x76;/* 远程获取参数 */
    int MINOR_REMOTE_CFG_PARM = 0x77;/* 远程配置参数 */
    int MINOR_REMOTE_GET_STATUS = 0x78;/* 远程获取状态 */
    int MINOR_REMOTE_ARM = 0x79; /* 远程布防 */
    int MINOR_REMOTE_DISARM = 0x7a;/* 远程撤防 */
    int MINOR_REMOTE_REBOOT = 0x7b; /* 远程重启 */
    int MINOR_START_VT = 0x7c;/* 开始语音对讲 */
    int MINOR_STOP_VT = 0x7d;/* 停止语音对讲 */
    int MINOR_REMOTE_UPGRADE = 0x7e; /* 远程升级 */
    int MINOR_REMOTE_PLAYBYFILE = 0x7f; /* 远程按文件回放 */
    int MINOR_REMOTE_PLAYBYTIME = 0x80; /* 远程按时间回放 */
    int MINOR_REMOTE_PTZCTRL = 0x81; /* 远程云台控制 */
    int MINOR_REMOTE_FORMAT_HDD = 0x82;  /* 远程格式化硬盘 */
    int MINOR_REMOTE_STOP = 0x83;  /* 远程关机 */
    int MINOR_REMOTE_LOCKFILE = 0x84;/* 远程锁定文件 */
    int MINOR_REMOTE_UNLOCKFILE = 0x85;/* 远程解锁文件 */
    int MINOR_REMOTE_CFGFILE_OUTPUT = 0x86;   /* 远程导出配置文件 */
    int MINOR_REMOTE_CFGFILE_INTPUT = 0x87;   /* 远程导入配置文件 */
    int MINOR_REMOTE_RECFILE_OUTPUT = 0x88;   /* 远程导出录象文件 */
    int MINOR_REMOTE_DVR_ALARM = 0x89;    /* 远程手动清除和触发报警*/
    int MINOR_REMOTE_IPC_ADD = 0x8a;  /* 远程添加IPC */
    int MINOR_REMOTE_IPC_DEL = 0x8b;/* 远程删除IPC */
    int MINOR_REMOTE_IPC_SET = 0x8c; /* 远程设置IPC */
    int MINOR_REBOOT_VCA_LIB = 0x8d;		/*重启智能库*/

    /*日志附加信息*/
//主类型
    int MAJOR_INFORMATION = 0x4;   /*附加信息*/
//次类型
int MINOR_HDD_INFO = 0xa1;/*硬盘信息*/
    int MINOR_SMART_INFO = 0xa2;   /*SMART信息*/
    int MINOR_REC_START = 0xa3;   /*开始录像*/
    int MINOR_REC_STOP = 0xa4;/*停止录像*/
    int MINOR_REC_OVERDUE = 0xa5;/*过期录像删除*/
    int MINOR_LINK_START = 0xa6; // ivms，多路解码器等连接前端设备
    int MINOR_LINK_STOP = 0xa7;// ivms，多路解码器等断开前端设备　
...
int PARA_VIDEOOUT = 0x1;
    int PARA_IMAGE = 0x2;
    int PARA_ENCODE = 0x4;
    int PARA_NETWORK = 0x8;
    int PARA_ALARM = 0x10;
    int PARA_EXCEPTION = 0x20;
    int PARA_DECODER = 0x40; /*解码器*/
    int PARA_RS232 = 0x80;
    int PARA_PREVIEW = 0x100;
    int PARA_SECURITY = 0x200;
    int PARA_DATETIME = 0x400;
    int PARA_FRAMETYPE = 0x800;  /*帧格式*/
    int PARA_VCA_RULE = 0x1000;    //行为规则
//SDK_V222
//智能设备类型
int DS6001_HF_B = 60;//行为分析：DS6001-HF/B
    int DS6001_HF_P = 61;//车牌识别：DS6001-HF/P
    int DS6002_HF_B = 62;//双机跟踪：DS6002-HF/B
    int DS6101_HF_B = 63;//行为分析：DS6101-HF/B
    int IVMS_2000 = 64;//智能分析仪
    int DS9000_IVS = 65;//9000系列智能DVR
    int DS8004_AHL_A = 66;//智能ATM, DS8004AHL-S/A
    int DS6101_HF_P = 67;//车牌识别：DS6101-HF/P
//能力获取命令
int VCA_DEV_ABILITY = 0x100;//设备智能分析的总能力
    int VCA_CHAN_ABILITY = 0x110;//行为分析能力
//获取/设置大接口参数配置命令
//车牌识别（NET_VCA_PLATE_CFG）;
int NET_DVR_SET_PLATECFG = 150 ;//设置车牌识别参数

    int NET_DVR_GET_PLATECFG = 151;	//获取车牌识别参数
//行为对应（NET_VCA_RULECFG）
int NET_DVR_SET_RULECFG = 152;	//设置行为分析规则
    int NET_DVR_GET_RULECFG = 153;//获取行为分析规则,
//双摄像机标定参数（NET_DVR_LF_CFG）
int NET_DVR_SET_LF_CFG = 160;//设置双摄像机的配置参数
    int NET_DVR_GET_LF_CFG = 161;//获取双摄像机的配置参数
//智能分析仪取流配置结构
int NET_DVR_SET_IVMS_STREAMCFG = 162;	//设置智能分析仪取流参数
    int NET_DVR_GET_IVMS_STREAMCFG = 163;	//获取智能分析仪取流参数
//智能控制参数结构
int NET_DVR_SET_VCA_CTRLCFG = 164; //设置智能控制参数
    int NET_DVR_GET_VCA_CTRLCFG = 165;	 //获取智能控制参数
//屏蔽区域NET_VCA_MASK_REGION_LIST
int NET_DVR_SET_VCA_MASK_REGION = 166;	 //设置屏蔽区域参数
    int NET_DVR_GET_VCA_MASK_REGION = 167;	 //获取屏蔽区域参数
//ATM进入区域 NET_VCA_ENTER_REGION
int NET_DVR_SET_VCA_ENTER_REGION = 168; //设置进入区域参数
    int NET_DVR_GET_VCA_ENTER_REGION = 169;	 //获取进入区域参数
//标定线配置NET_VCA_LINE_SEGMENT_LIST
int NET_DVR_SET_VCA_LINE_SEGMENT = 170;	 //设置标定线
    int NET_DVR_GET_VCA_LINE_SEGMENT = 171;	 //获取标定线
// ivms屏蔽区域NET_IVMS_MASK_REGION_LIST
int NET_DVR_SET_IVMS_MASK_REGION = 172;	 //设置IVMS屏蔽区域参数
    int NET_DVR_GET_IVMS_MASK_REGION = 173;	 //获取IVMS屏蔽区域参数
// ivms进入检测区域NET_IVMS_ENTER_REGION
int NET_DVR_SET_IVMS_ENTER_REGION = 174; //设置IVMS进入区域参数
    int NET_DVR_GET_IVMS_ENTER_REGION = 175; //获取IVMS进入区域参数
    int NET_DVR_SET_IVMS_BEHAVIORCFG = 176;//设置智能分析仪行为规则参数
    int NET_DVR_GET_IVMS_BEHAVIORCFG = 177;	//获取智能分析仪行为规则参数
    int NET_DVR_GET_TRAVERSE_PLANE_DETECTION = 3360; //获取越界侦测配置

    int NET_DVR_GET_CARD_CFG = 2116; //获取卡参数
    int NET_DVR_SET_CARD_CFG = 2117; //设置卡参数
    /**********************设备类型 end***********************/
    int NET_DVR_BARRIERGATE_CTRL = 3128;//远程控制道闸
    int NET_DVR_GATELAMP_CTRL = 3129;//常亮灯功能
...
class NET_DVR_TIME extends Structure {//校时结构参数
    ...
    class NET_DVR_SCHEDTIME extends Structure {
  ...
  class NET_DVR_HANDLEEXCEPTION_V30 extends Structure {
...
//报警和异常处理结构(子结构)(多处使用)
class NET_DVR_HANDLEEXCEPTION extends Structure {
	public int	dwHandleType;			/*处理方式,处理方式的"或"结果*//*0x00: 无响应*//*0x01: 监视器上警告*//*0x02: 声音警告*//*0x04: 上传中心*/	/*0x08: 触发报警输出*//*0x20: 触发抓图*/  //(JPEG定制)
	public byte[]  byRelAlarmOut = new byte[MAX_ALARMOUT];  //报警触发的输出通道,报警触发的输出,为1表示触发该输出
}

//DVR设备参数
class NET_DVR_DEVICECFG extends Structure {
        public int dwSize;
...
//网络数据结构(子结构)(9000扩展)
class NET_DVR_ETHERNET_V30 extends Structure {
    ...
    class NET_DVR_ETHERNET extends Structure {//网络数据结构(子结构)
    ...
    class NET_DVR_PPPOECFG extends Structure {//PPPoe
        public int dwPPPoE;
 ...
 class NET_DVR_NETCFG_V30 extends Structure {
        public int dwSize;
 ...
 class NET_DVR_NETCFG extends Structure {//网络配置结构
	public int dwSize;
...
//通道图象结构
class NET_DVR_SCHEDTIMEWEEK extends Structure {
        public NET_DVR_SCHEDTIME[] struAlarmTime = new NET_DVR_SCHEDTIME[8];
    }

     class byte96 extends Structure {
        public byte[] byMotionScope = new byte[96];
    }
     
  class NET_DVR_MOTION_V30 extends Structure {//移动侦测(子结构)(9000扩展)
  ...
  class NET_DVR_MOTION extends Structure {//移动侦测(子结构)
  ...
  class NET_DVR_HIDEALARM_V30 extends Structure {//遮挡报警
  ...
  class NET_DVR_HIDEALARM extends Structure {//遮挡报警(子结构)  区域大小704*576
	public int dwEnableHideAlarm;				/* 是否启动遮挡报警 ,0-否,1-低灵敏度 2-中灵敏度 3-高灵敏度*/
	public short wHideAlarmAreaTopLeftX;			/* 遮挡区域的x坐标 */
	public short wHideAlarmAreaTopLeftY;			/* 遮挡区域的y坐标 */
	public short wHideAlarmAreaWidth;				/* 遮挡区域的宽 */
...
class NET_DVR_VILOST extends Structure {    //信号丢失报警(子结构)
    ...
    class NET_DVR_COLOR extends Structure {
    ...
    class NET_DVR_VICOLOR extends Structure {
    ...
    class NET_DVR_PICCFG_V30 extends Structure {
        public int dwSize;
    ...
    class NET_DVR_PICCFG_EX extends Structure {//通道图象结构SDK_V14扩展
	public int dwSize;
	 public byte[] sChanName = new byte[NAME_LEN];
	 public int dwVideoFormat;	/* 只读 视频制式 1-NTSC 2-PAL*/
	 public byte byBrightness;  	/*亮度,0-255*/
 ...
 class NET_DVR_PICCFG extends Structure { //通道图象结构(SDK_V13及之前版本)
	 public int dwSize;
	 public byte[] sChanName = new byte[NAME_LEN];
	 public int dwVideoFormat;	/* 只读 视频制式 1-NTSC 2-PAL*/
	 public byte byBrightness;  	/*亮度,0-255*/
    ...
    //码流压缩参数(子结构)(9000扩展)
    class NET_DVR_COMPRESSION_INFO_V30 extends Structure {
    ...
    //通道压缩参数(9000扩展)
    class NET_DVR_COMPRESSIONCFG_V30 extends Structure {
        public int dwSize;
    ...
    class NET_DVR_COMPRESSION_INFO extends Structure {//码流压缩参数(子结构)
    ...
    class NET_DVR_COMPRESSIONCFG extends Structure {//通道压缩参数
	public int dwSize;
    ...
    class NET_DVR_COMPRESSION_INFO_EX extends Structure {//码流压缩参数(子结构)(扩展) 增加I帧间隔
    ...
    class NET_DVR_RECORDDAY extends Structure //全天录像参数配置(子结构)
    {
    ...
    class NET_DVR_RECORDSCHEDWEEK extends Structure
    {
       public 	NET_DVR_RECORDSCHED[] struRecordSched = new NET_DVR_RECORDSCHED[MAX_TIMESEGMENT_V30];
    }

    class NET_DVR_RECORD_V30 extends Structure {    //通道录像参数配置(9000扩展)
        public int dwSize;
 ...
 class NET_DVR_RECORD extends Structure { //通道录像参数配置
	 public int dwSize;
	 public int dwRecord;  /*是否录像 0-否 1-是*/
	 public NET_DVR_RECORDDAY[]  struRecAllDay = new NET_DVR_RECORDDAY[MAX_DAYS];
         public NET_DVR_RECORDSCHEDWEEK[] struRecordSched = new NET_DVR_RECORDSCHEDWEEK[MAX_DAYS];
...
class NET_DVR_STATFRAME extends Structure { //单帧统计参数
	 public int dwRelativeTime;
	 public int dwAbsTime;  /*统计绝对时标*/
         public byte[] byRes = new byte[92];
}

class NET_DVR_STATTIME extends Structure { //单帧统计参数
...
class NET_DVR_PDC_ALRAM_INFO extends Structure { //通道录像参数配置
	 public int dwSize;
...
/***************************云台类型(end)******************************/
class NET_DVR_DECODERCFG_V30 extends Structure {//通道解码器(云台)参数配置(9000扩展)
	public int dwSize;
 ...
 class NET_DVR_DECODERCFG extends Structure {//通道解码器(云台)参数配置
	public int dwSize;
	public int dwBaudRate;       //波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－19200， 11－38400，12－57600，13－76800，14－115.2k;
	public byte byDataBit;         // 数据有几位 0－5位，1－6位，2－7位，3－8位;
	public byte byStopBit;         // 停止位 0－1位，1－2位;
...
class NET_DVR_PPPCFG_V30 extends Structure {//ppp参数配置(子结构)
...
class NET_DVR_PPPCFG extends Structure {//ppp参数配置(子结构)
...
class NET_DVR_SINGLE_RS232 extends Structure {//RS232串口参数配置(9000扩展)
       public int dwBaudRate;   /*波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－19200， 11－38400，12－57600，13－76800，14－115.2k;*/
       public byte byDataBit;     /* 数据有几位 0－5位，1－6位，2－7位，3－8位 */
       public byte byStopBit;     /* 停止位 0－1位，1－2位 */
       public byte byParity;      /* 校验 0－无校验，1－奇校验，2－偶校验 */
...
class NET_DVR_RS232CFG_V30 extends Structure {//RS232串口参数配置(9000扩展)
	public int dwSize;
...
class NET_DVR_RS232CFG extends Structure {//RS232串口参数配置
	public int dwSize;
	public int dwBaudRate;//波特率(bps)，0－50，1－75，2－110，3－150，4－300，5－600，6－1200，7－2400，8－4800，9－9600，10－19200， 11－38400，12－57600，13－76800，14－115.2k;
	public byte byDataBit;// 数据有几位 0－5位，1－6位，2－7位，3－8位;
	public byte byStopBit;// 停止位 0－1位，1－2位;
...
class NET_DVR_ALARMINCFG_V30 extends Structure {//报警输入参数配置(9000扩展)
        public 	int dwSize;
...
class NET_DVR_ALARMINCFG extends Structure {//报警输入参数配置
	public int dwSize;
	public byte[] sAlarmInName = new byte[NAME_LEN];	/* 名称 */
	public byte byAlarmType;	//报警器类型,0：常开,1：常闭
	public byte byAlarmInHandle;	/* 是否处理 0-不处理 1-处理*/
...
class NET_DVR_ADDIT_POSITION extends Structure {//车载GPS信息结构(2007-12-27)
...
class NET_DVR_ALARMINFO extends Structure {
...
class NET_DVR_ALARMINFO_EX extends Structure {//上传报警信息(杭州竞天定制 2006-07-28)
...
class NET_DVR_SINGLE_HD extends Structure {//本地硬盘信息配置
...
class NET_DVR_SCALECFG extends Structure {//配置缩放参数的结构
       public  int dwSize;
...
class NET_DVR_ALARMOUTCFG_V30 extends Structure {//DVR报警输出(9000扩展)
	public int dwSize;
...
class NET_DVR_ALARMOUTCFG extends Structure {//DVR报警输出
	public int dwSize;
	public byte[] sAlarmOutName = new byte[NAME_LEN];	/* 名称 */
	public int dwAlarmOutDelay;	/* 输出保持时间(-1为无限，手动关闭) */
	//0-5秒,1-10秒,2-30秒,3-1分钟,4-2分钟,5-5分钟,6-10分钟,7-手动
...
class NET_DVR_PREVIEWCFG_V30 extends Structure {//DVR本地预览参数(9000扩展)
       public  int dwSize;
...
class NET_DVR_PREVIEWCFG extends Structure {//DVR本地预览参数
	public int dwSize;
...
class NET_DVR_VGAPARA extends Structure {//DVR视频输出
...
* MATRIX输出参数结构
*/
class NET_DVR_MATRIXPARA_V30 extends Structure {
...
class NET_DVR_MATRIXPARA extends Structure {
	public short wDisplayLogo;						/* 显示视频通道号(保留) */
	public short wDisplayOsd;						/* 显示时间(保留) */
}

class NET_DVR_VOOUT extends Structure {
...
class NET_DVR_VIDEOOUT_V30 extends Structure {//DVR视频输出(9000扩展)
	public int dwSize;
...
class NET_DVR_VIDEOOUT extends Structure {//DVR视频输出
	public int dwSize;
...
class NET_DVR_USER_INFO_V30 extends Structure {//单用户参数(子结构)(9000扩展)
	public byte[] sUserName = new byte[NAME_LEN];		/* 用户名 */
	public byte[] sPassword = new byte[PASSWD_LEN];		/* 密码 */
...
class NET_DVR_USER_INFO_EX extends Structure {//单用户参数(SDK_V15扩展)(子结构)
	public byte[] sUserName = new byte[NAME_LEN];		/* 用户名 */
	public byte[] sPassword = new byte[PASSWD_LEN];		/* 密码 */
...
class NET_DVR_USER_INFO extends Structure {//单用户参数(子结构)
	public byte[] sUserName = new byte[NAME_LEN];		/* 用户名 */
	public byte[] sPassword = new byte[PASSWD_LEN];		/* 密码 */
	public int[] dwLocalRight = new int[MAX_RIGHT];	/* 权限 */
	/*数组0: 本地控制云台*/
...
class NET_DVR_EXCEPTION extends Structure {//DVR异常参数
	public int dwSize;
...
class NET_DVR_CHANNELSTATE extends Structure {//通道状态
	public byte byRecordStatic; //通道是否在录像,0-不录像,1-录像
	public byte bySignalStatic; //连接的信号状态,0-正常,1-信号丢失
	public byte byHardwareStatic;//通道硬件状态,0-正常,1-异常,例如DSP死掉
	public byte reservedData;		//保留
...
class NET_DVR_WORKSTATE extends Structure {//DVR工作状态
	public int dwDeviceStatic; 	//设备的状态,0-正常,1-CPU占用率太高,超过85%,2-硬件错误,例如串口死掉
...
class NET_DVR_LOG_V30 extends Structure {//日志信息(9000扩展)
...
//日志信息
class NET_DVR_LOG extends Structure {
	public NET_DVR_TIME strLogTime;
	public int	dwMajorType;	//主类型 1-报警; 2-异常; 3-操作; 0xff-全部
	public int	dwMinorType;//次类型 0-全部;
	public byte[]	sPanelUser = new byte[MAX_NAMELEN]; //操作面板的用户名
...
/************************DVR日志 end***************************/
class NET_DVR_ALARMOUTSTATUS_V30 extends Structure {//报警输出状态(9000扩展)
	public byte[] Output = new byte[MAX_ALARMOUT_V30];
}

class NET_DVR_ALARMOUTSTATUS extends Structure {//报警输出状态
	public byte[] Output = new byte[MAX_ALARMOUT];
}

class NET_DVR_TRADEINFO extends Structure {//交易信息
	public short m_Year;
...
class NET_DVR_FRAMETYPECODE extends Structure {/*帧格式*/
	public byte[] code = new byte[12];		/* 代码 */
}

class NET_DVR_FRAMEFORMAT_V30 extends Structure {//ATM参数(9000扩展)
	public int	dwSize;
...
class NET_DVR_FRAMEFORMAT extends Structure {//ATM参数
	public int dwSize;
...
class NET_DVR_FTPTYPECODE extends Structure {
	public byte[] sFtpType = new byte[32];     /*客户定义的操作类型*/
	public byte[] sFtpCode = new byte[8];      /*客户定义的操作类型的对应的码*/
}

class NET_DVR_FRAMEFORMAT_EX extends Structure {//ATM参数添加FTP上传参数, 俄罗斯银行定制, 2006-11-17
	public int dwSize;
	public byte[] sATMIP = new byte[16];						/* ATM IP地址 */
	public int dwATMType;						/* ATM类型 */
	public int dwInputMode;						/* 输入方式	0-网络侦听 1-网络接收 2-串口直接输入 3-串口ATM命令输入*/
...
/*****************************DS-6001D/F(begin)***************************/
//DS-6001D Decoder
class NET_DVR_DECODERINFO extends Structure {
...
class NET_DVR_DECODERSTATE extends Structure {
	public byte[] byEncoderIP = new byte[16];		//解码设备连接的服务器IP
	public byte[] byEncoderUser = new byte[16];		//解码设备连接的服务器的用户名
	public byte[] byEncoderPasswd = new byte[16];	//解码设备连接的服务器的密码
	public byte byEncoderChannel;		//解码设备连接的服务器的通道号
...
class NET_DVR_DECCHANINFO extends Structure {
	public byte[] sDVRIP = new byte[16];				/* DVR IP地址 */
	public short wDVRPort;			 		/* 端口号 */
	public byte[] sUserName = new byte[NAME_LEN];		/* 用户名 */
	public byte[] sPassword = new byte[PASSWD_LEN];		/* 密码 */
...
class NET_DVR_DECINFO extends Structure {/*每个解码通道的配置*/
...
class NET_DVR_DECCFG extends Structure {/*整个设备解码配置*/
	public int	dwSize;
...
//2005-08-01
class NET_DVR_PORTINFO extends Structure {/* 解码设备透明通道设置 */
...
class NET_DVR_PORTCFG extends Structure {
	public int dwSize;
...
class NET_DVR_PLAYREMOTEFILE extends Structure {/* 控制网络文件回放 */
	public int dwSize;
	public byte[] sDecoderIP = new byte[16];		/* DVR IP地址 */
	public short wDecoderPort;			/* 端口号 */
...
class NET_DVR_DECCHANSTATUS extends Structure {/*当前设备解码连接状态*/
...
class NET_DVR_DECSTATUS extends Structure {
	public int   dwSize;
	public NET_DVR_DECCHANSTATUS[] struDecState = new NET_DVR_DECCHANSTATUS[MAX_DECNUM];
}
/*****************************DS-6001D/F(end)***************************/

class NET_DVR_SHOWSTRINGINFO extends Structure {//单字符参数(子结构)
...
DVR实现巡航数据结构
*/
class NET_DVR_CRUISE_PARA extends Structure {
	public int 	dwSize;
...
//时间点
    class NET_DVR_TIMEPOINT extends Structure {
...
//夏令时参数
class NET_DVR_ZONEANDDST extends Structure {
	public int dwSize;
...
//辅助输出参数配置
class NET_DVR_AUXOUTCFG extends Structure {
	public int dwSize;
...
//ntp
class NET_DVR_NTPPARA extends Structure {
...
//ddns
class NET_DVR_DDNSPARA extends Structure {
   ...
   class NET_DVR_DDNSPARA_EX extends Structure {
   ...
   class NET_DVR_DDNS extends Structure {
...
//9000扩展
class NET_DVR_DDNSPARA_V30 extends Structure {
...
//email
class NET_DVR_EMAILPARA extends Structure {
...
class NET_DVR_NETAPPCFG extends Structure {//网络参数配置
	public int  dwSize;
...
class NET_DVR_MATRIX_DECINFO extends Structure {
	public byte[] 	sDVRIP = new byte[16];				/* DVR IP地址 */
	public short 	wDVRPort;			 	/* 端口号 */
	public byte 	byChannel;				/* 通道号 */
...
class NET_DVR_MATRIX_DYNAMIC_DEC extends Structure {//启动/停止动态解码
	public int	dwSize;
	public NET_DVR_MATRIX_DECINFO struDecChanInfo;		/* 动态解码通道信息 */
}

class NET_DVR_MATRIX_DEC_CHAN_STATUS extends Structure {//2007-12-13 modified by zxy 修改多路解码器的NET_DVR_MATRIX_DEC_CHAN_STATUS结构
...
class NET_DVR_MATRIX_DEC_CHAN_INFO extends Structure {
	public int	dwSize;
...
//矩阵行信息 2007-12-28
class NET_DVR_MATRIX_ROW_ELEMENT extends Structure {
...
class NET_DVR_MATRIX_ROW_INDEX extends Structure {
	public byte[]	sSurvChanName = new byte[128];			/* 监控通道名称，支持中文 */
	public int	dwRowNum;				/* 行号 */
}

//矩阵列信息 2007-12-28
class NET_DVR_MATRIX_COLUMN_ELEMENT extends Structure {
	public int  dwLocalDispChanNum;	/* 本地显示通道号 */
	public int  dwGlobalDispChanNum;	/* 全局显示通道号 */
	public int  dwRes;			/* 保留 */
}

class NET_DVR_MATRIX_GLOBAL_COLUMN_ELEMENT extends Structure {
...
//手动查看 2007-12-28
class NET_DVR_MATRIX_ROW_COLUMN_LINK extends Structure {
	public int	dwSize;
	/*
...
class NET_DVR_MATRIX_PREVIEW_DISP_CHAN extends Structure {
	public int		dwSize;
	public int		dwGlobalDispChanNum;		/* 电视墙上的电视机编号 */
	public int		dwLocalDispChanNum;		/* 解码通道 */
}

class NET_DVR_MATRIX_LOOP_PLAY_SET extends Structure {//轮循功能 2007-12-28
	public int	dwSize;
...
class NET_DVR_MATRIX_LOCAL_HOST_INFO extends Structure {//矩阵中心配置 2007-12-28
	public int	dwSize;
...
//2007-12-22
class TTY_CONFIG extends Structure {
...
class NET_DVR_MATRIX_TRAN_CHAN_INFO extends Structure {
...
class NET_DVR_MATRIX_TRAN_CHAN_CONFIG extends Structure {
        public 	int dwSize;
...
class NET_DVR_MATRIX_DEC_REMOTE_PLAY extends Structure {
	public int	dwSize;
	public byte[]	sDVRIP = new byte[16];		/* DVR IP地址 */
	public short	wDVRPort;			/* 端口号 */
	public byte	byChannel;			/* 通道号 */
...
class NET_DVR_MATRIX_DEC_REMOTE_PLAY_CONTROL extends Structure {
	public int	dwSize;
	public int	dwPlayCmd;		/* 播放命令 见文件播放命令*/
	public int	dwCmdParam;		/* 播放命令参数 */
}

class NET_DVR_MATRIX_DEC_REMOTE_PLAY_STATUS extends Structure {
	public int dwSize;
...
class NET_DVR_MATRIX_PASSIVEMODE extends Structure {
...
class NET_DVR_EMAILCFG  extends Structure
{	/* 12 bytes */
	public int	dwSize;
...
class NET_DVR_COMPRESSIONCFG_NEW extends Structure
{
	public int dwSize;
...
//球机位置信息
class NET_DVR_PTZPOS extends Structure
{
...
//球机范围信息
class NET_DVR_PTZSCOPE extends Structure
{
...
//rtsp配置 ipcamera专用
class NET_DVR_RTSPCFG extends Structure
{
...
//NET_DVR_Login()参数结构
class NET_DVR_DEVICEINFO extends Structure
{
	public byte[] sSerialNumber = new byte[SERIALNO_LEN];   //序列号
	public byte byAlarmInPortNum;		        //DVR报警输入个数
	public byte byAlarmOutPortNum;		        //DVR报警输出个数
...
//SDK功能支持信息(9000新增)
class NET_DVR_SDKABL extends Structure
{
...
//硬解码显示区域参数(子结构)
class NET_DVR_DISPLAY_PARA extends Structure
{
...
//硬解码预览参数
class NET_DVR_CARDINFO extends Structure
{
...
//录象文件参数
class NET_DVR_FIND_DATA extends Structure
{
...
//录象文件参数(9000)
class NET_DVR_FINDDATA_V30 extends Structure {
        public byte[] sFileName = new byte[100];//文件名
        public NET_DVR_TIME struStartTime;//文件的开始时间
        public NET_DVR_TIME struStopTime;//文件的结束时间
        public int dwFileSize;//文件的大小
...
//录象文件参数(带卡号)
class NET_DVR_FINDDATA_CARD extends Structure
{
	public byte[] sFileName = new byte[100];//文件名
	public NET_DVR_TIME struStartTime;//文件的开始时间
	public NET_DVR_TIME struStopTime;//文件的结束时间
 ...
 class NET_DVR_FILECOND extends Structure //录象文件查找条件结构
    {
        public NativeLong lChannel;//通道号
...
//云台区域选择放大缩小(HIK 快球专用)
class NET_DVR_POINT_FRAME extends Structure
{
...
//语音对讲参数
class NET_DVR_COMPRESSION_AUDIO extends Structure
{
...
class NET_VCA_TRAVERSE_PLANE extends Structure
{
 ...
 class NET_DVR_HANDLEEXCEPTION_V40 extends Structure
 {
...
class NET_VCA_TRAVERSE_PLANE_DETECTION extends Structure
{
    public int dwSize;
...
class NET_DVR_CHANNEL_GROUP extends Structure
{
    public int dwSize;
    public int dwChannel;
    public int dwGroup;
...
//车牌识别结果子结构体
class NET_DVR_PLATE_INFO extends Structure
{
...
//车辆信息参数
class NET_DVR_VEHICLE_INFO extends Structure
 {
	   public int dwIndex;
...
//交通抓拍结果信息
class NET_DVR_PLATE_RESULT extends Structure
{
	  public int dwSize;
...
class NET_DVR_TIME_V30 extends Structure
{
          public short wYear;
	  public byte byMonth;
	  public byte byDay;
...
class NET_ITS_PICTURE_INFO extends Structure
{
...
class NET_ITS_PLATE_RESULT extends Structure
 {
	  public int dwSize;
...
//停车场数据上传
class NET_ITS_PARK_VEHICLE extends Structure
{
...
class NET_DVR_SNAPCFG extends Structure {
...
//卡参数配置条件
class NET_DVR_CARD_CFG_COND extends Structure {
      public int dwSize;
...
//获取卡参数的发送数据
class NET_DVR_CARD_CFG_SEND_DATA extends Structure {
      public int dwSize;
      public byte[] byCardNo = new byte[32];
      public byte[] byRes = new byte[16];
}

class CARDRIGHTPLAN extends Structure {
   public byte[] byRightPlan = new byte[4];
}

//卡参数
class NET_DVR_CARD_CFG extends Structure {
	public int dwSize;
...
//有效期参数结构体
class NET_DVR_VALID_PERIOD_CFG extends Structure {
...
//门禁主机报警信息结构体
class NET_DVR_ACS_ALARM_INFO extends Structure {
	public int dwSize;
	public int dwMajor;
	public int dwMinor;
...
//门禁主机事件信息
class NET_DVR_ACS_EVENT_INFO extends Structure {
	public int dwSize;
	public byte[] byCardNo = new byte[32];
	public byte byCardType;
...
class NET_DVR_STRING_POINTER extends Structure {
...
//报警场景信息
class NET_DVR_SCENE_INFO  extends Structure {
...
class NET_DVR_TFS_ALARM  extends Structure {
	public int dwSize;
	public int dwRelativeTime;
	public int dwAbsTime;
...
//人体特征识别结果结构体
class NET_VCA_HUMAN_FEATURE  extends Structure {
...
//人脸抓拍结果
class NET_VCA_FACESNAP_RESULT  extends Structure {
	public int dwSize;             // 结构大小
...
//人脸抓拍信息
class NET_VCA_FACESNAP_INFO_ALARM extends Structure {
	public int dwRelativeTime;     // 相对时标
	public int dwAbsTime;            // 绝对时标
...
//籍贯参数
class NET_DVR_AREAINFOCFG extends Structure {
...
class NET_VCA_HUMAN_ATTRIBUTE extends Structure {
...
//黑名单信息
class NET_VCA_BLACKLIST_INFO extends Structure {
	public int dwSize;   //结构大小
...
//黑名单报警信息
class NET_VCA_BLACKLIST_INFO_ALARM extends Structure {
...
//黑名单比对结果报警上传
class NET_VCA_FACESNAP_MATCH_ALARM extends Structure {
	public int dwSize;             // 结构大小
...
//道闸控制参数结构体。
class NET_DVR_BARRIERGATE_CFG extends Structure {
	 public int dwSize;
	 public int dwChannel;
	 public byte byLaneNo;
 ...
 interface FRealDataCallBack_V30 extends StdCallCallback {
   ...
   interface FMessageCallBack extends StdCallCallback {
      ...
      interface FDrawFun extends StdCallCallback {
        void invoke(NativeLong lRealHandle, W32API.HDC hDc, int dwUser);
      }
...
class NET_DVR_GPS_DATA extends Structure {
 ...
 interface fGPSDataCallback extends StdCallCallback {
       void invoke(NativeLong nHandle, int dwState, Pointer lpBuffer, int dwBufLen, Pointer pUser);
 }
...
{
    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary("E:\\HCNetSDK32\\PlayCtrl",
            PlayCtrl.class);
