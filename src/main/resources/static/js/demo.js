...
function clickLogin() {
	...
	$.ajax({
		url:"/graindepot-video/video/test",
		type:"post",
		data:{"username":szUsername,"password":szPassword,"m_sDeviceIP":szIP,"deviceport":deviceport},
		success: function(data){
			if(data>0){
				showOPInfo(szIP + " 后台登录成功！");
			}
		},
			error: function () {
				showOPInfo(szIP + " 后台登录失败！");
			}
		});
}
...
//		url:"/a/test",
//		type:"post",
//		data:{"username":szUsername,"password":szPassword,"m_sDeviceIP":szIP},
//		success: function(data){
//			if(data>0){
//				showOPInfo(szIP + " 登录成功！");
//				$("#ip").prepend("<option value='" + szIP + "'>" + szIP + "</option>");
...
function getChannelInfo() {
	...
	WebVideoCtrl.I_GetAnalogChannelInfo(szIP, {
		...
		success: function (xmlDoc) {
			...
			//如果所登录设备为相机，当获取的模拟通道数为0时，这里暂时写死为一个；
			if(nAnalogChannel == 0)
			{
				name="Camera 01";
				oSel.append("<option value='1'" + " bZero='false'>" + name + "</option>");
			}
...
function clickStartRealPlay() {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		...
		szInfo = "";
	if($("#channels").val() == null || $("#channels").val() == ''){
		iChannelID = 1;
	}
			...
			if("MORE" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
				...
				for(var i = 0, nLen = $(xmlDoc).find("searchMatchItem").length; i < nLen; i++) {	
					...
					if(szPlaybackURI.indexOf("name=") < 0) {
						//break;
						var newXmlDoc = $(xmlDoc).find("searchMatchItem")[i];
						var newURI = reBuildURI(newXmlDoc);
						szPlaybackURI = newURI.uri;						
					}
...
//重新组装回放的URI(参照终端录像回放和下载代码)
function reBuildURI(xmlDoc) {
	var $ele = $(xmlDoc);
	var uri = $ele.find('playbackURI').text();
	var startTime = $ele.find('startTime').text();
	var endTime = $ele.find('endTime').text();
	var uriSegs = uri.split('&');
	var uri0 = uriSegs[0];

	var fileName = uri0.substr(uri0.lastIndexOf('/')+1);
	var rtsp = uri0.substring(0,uri0.lastIndexOf('/')+1);
	var size = 1000;
	for (var i = 1; i < uriSegs.length; i++) {
		if (uriSegs[i].indexOf('size') >= 0) {
			size = uriSegs[i].replace(/\D/gi, '');
			break;
		}
	}

	var st = startTime.replace(/-|:/gi, '');
	var et = endTime.replace(/-|:/gi, '');
	...
	var objStr = {
		...
		fileName: fileName,
		size: size,
		uri:rtsp+'?starttime='+st+'&endtime='+et+'&name='+fileName+'&size='+size+'&loadType=0'
	};
	return objStr;
}
