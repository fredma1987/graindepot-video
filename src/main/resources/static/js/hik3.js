...
function getWindowSize() {
    var nWidth = $(this).width() + $(this).scrollLeft(), nHeight = $(this)
            .height()
        + $(this).scrollTop();
...
function clickGetLocalCfg() {
    ...
    $("#playbackPicPath").val(
        $(xmlDoc).find("PlaybackPicPath").eq(0).text());
    $("#playbackFilePath").val(
        $(xmlDoc).find("PlaybackFilePath").eq(0).text());
 ...
 * 登录
 * @param szIP ip地址
 * @param szPort http端口号
 * @param iChannelID 通道号
 * @param bZeroChannel 使用零通道
 * @param szUsername 用户名
 * @param szPassword 密码
 */
function clickLogin(szIP,szPort,iChannelID,bZeroChannel,szUsername,szPassword,iStreamType) {
    /*var szIP = document.getElementById("DeviceIP").value,
        szPort = document.getElementById("DevicePort").value,
        szUsername = document.getElementById("DeviceUsername").value,
        szPassword = document.getElementById("DevicePasswd").value;*/
    //判断是ip,若不是IP则将域名转为IP
    if (!reg.test(szIP)) {
        szIP=szIP.replace("http://","");
        var arr=szIP.split("/");
        szDeviceInfo = WebVideoCtrl.I_GetIPInfoByMode(2, arr[0], 80, arr[1]);
...
function clickLogout() {
    var szIP = document.getElementById("DeviceIP").value, szInfo = "";
    ...
    WebVideoCtrl.I_GetDeviceInfo(szIP,
        {
            success : function(xmlDoc) {
                var arrStr = [];
                arrStr.push("设备名称："
                    + $(xmlDoc).find("deviceName").eq(0).text()
                    + "\r\n");
                arrStr.push("设备ID："
                    + $(xmlDoc).find("deviceID").eq(0).text()
                    + "\r\n");
                arrStr
                    .push("型号："
                    + $(xmlDoc).find("model").eq(0).text()
                    + "\r\n");
                arrStr.push("设备序列号："
                    + $(xmlDoc).find("serialNumber").eq(0).text()
                    + "\r\n");
                arrStr.push("MAC地址："
                    + $(xmlDoc).find("macAddress").eq(0).text()
                    + "\r\n");
                arrStr.push("主控版本："
                    + $(xmlDoc).find("firmwareVersion").eq(0)
                        .text()
                    + " "
                    + $(xmlDoc).find("firmwareReleasedDate").eq(0)
                        .text() + "\r\n");
                arrStr.push("编码版本："
                    + $(xmlDoc).find("encoderVersion").eq(0).text()
                    + " "
                    + $(xmlDoc).find("encoderReleasedDate").eq(0)
                        .text() + "\r\n");
...
function getChannelInfo() {
    ...
    /*WebVideoCtrl
     ...
     oSel
     .append("<option value='" + id + "' bZero='false'>"
     + name
     + "</option>");
     });
     //alert(szIP + " 获取模拟通道成功！");
    ...
    /*WebVideoCtrl
     ...
     .text()) {// 过滤禁用的零通道
     oSel
     .append("<option value='" + id + "' bZero='true'>"
     + name
     + "</option>");
     }
     });
...
function clickGetDigitalChannelInfo() {
    var szIP = document.getElementById("DeviceIP").value, iAnalogChannelNum = 0;
...
function clickCapturePic() {
    ...
    if (oWndInfo != null) {
        var szChannelID = $("#channels").val(), szPicName = oWndInfo.szIP
            + "_" + szChannelID + "_" + new Date().getTime(), iRet = WebVideoCtrl
...
function clickStartRecord() {
    ...
    if (oWndInfo != null) {
        var szChannelID = $("#channels").val(), szFileName = oWndInfo.szIP
            + "_" + szChannelID + "_" + new Date().getTime(), iRet = WebVideoCtrl
...
// 获取对讲通道
function clickGetAudioInfo(szIP) {
    //var szIP = document.getElementById("DeviceIP").value;
...
function clickStartVoiceTalk() {
    var szIP = document.getElementById("DeviceIP").value, iAudioChannel = parseInt(
        $("#audiochannels").val(), 10), szInfo = "";
...
function clickStopVoiceTalk() {
    var szIP = document.getElementById("DeviceIP").value, iRet = WebVideoCtrl
        .I_StopVoiceTalk(), szInfo = "";
...
function mouseDownPTZControl(iPTZIndex) {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex), bZeroChannel = $(
        "#channels option").eq($("#channels").get(0).selectedIndex)
        .attr("bZero") == "true" ? true : false, iPTZSpeed = $(
...
function clickRecordSearch(iType) {
    var szIP = document.getElementById("DeviceIP").value, iChannelID = $(
        "#channels").val(), bZeroChannel = $("#channels option").eq(
        $("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true
        : false, szStartTime = $("#starttime").val(), szEndTime = $(
        "#endtime").val();
    ...
    WebVideoCtrl
        ...
        szEndTime,
        {
            ...
            success : function(xmlDoc) {
                if ("MORE" === $(xmlDoc).find(
                    ...
                    for ( var i = 0, nLen = $(xmlDoc).find(
                        ...
                        var szStartTime = $(xmlDoc).find(
                            "startTime").eq(i).text();
                        var szEndTime = $(xmlDoc).find(
                            "endTime").eq(i).text();
                        var szFileName = szPlaybackURI
                            .substring(
                            szPlaybackURI
                                .indexOf("name=") + 5,
                            szPlaybackURI
                        ...
                        var objTr = $("#searchlist").get(0)
                            .insertRow(-1);
                        var objTd = objTr.insertCell(0);
                        objTd.id = "downloadTd" + i;
                        objTd.innerHTML = iSearchTimes * 40
                            + (i + 1);
                        objTd = objTr.insertCell(1);
                        objTd.width = "30%";
                        ...
                        objTd = objTr.insertCell(2);
                        objTd.width = "30%";
                        objTd.innerHTML = (szStartTime.replace(
                            "T", " ")).replace("Z", "");
                        objTd = objTr.insertCell(3);
                        objTd.width = "30%";
                        objTd.innerHTML = (szEndTime.replace(
                            "T", " ")).replace("Z", "");
                        objTd = objTr.insertCell(4);
                        objTd.width = "10%";
                        objTd.innerHTML = "<a href='javascript:;' onclick='clickStartDownloadRecord("
                            + i + ");'>下载</a>";
                        $("#downloadTd" + i).data(
                ...
                } else if ("OK" === $(xmlDoc).find(
                        "responseStatusStrg").eq(0).text()) {
                    var iLength = $(xmlDoc).find(
                        "searchMatchItem").length;
...
function clickStartPlayback() {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
        szIP = document.getElementById("DeviceIP").value,
        iChannelID = document.getElementById("ChannelList").value,
        bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false,
        szStartTime = $("#begin").val(),
        szEndTime = $("#end").val(),
        szInfo = "",
...
function clickReversePlayback() {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex), szIP = document
        .getElementById("DeviceIP").value, bZeroChannel = $(
        "#channels option").eq($("#channels").get(0).selectedIndex)
        .attr("bZero") == "true" ? true : false, iChannelID = $(
        "#channels").val(), szStartTime = $("#starttime").val(), szEndTime = $(
        "#endtime").val(), szInfo = "";
...
function clickGetOSDTime() {
    ...
    if (oWndInfo != null) {
        ...
        if (szTime != -1) {
            $("#osdtime").val(szTime);
            //alert(oWndInfo.szIP + " 获取OSD时间成功！");
        } else {
            //alert(oWndInfo.szIP + " 获取OSD时间失败！");
        }
    }
}
...
function clickStartDownloadRecord(i) {
    var szIP = document.getElementById("DeviceIP").value, szChannelID = $(
        "#channels").val(), szFileName = szIP + "_" + szChannelID + "_"
        + new Date().getTime(), szPlaybackURI = $("#downloadTd" + i)
        .data("playbackURI");
...
function clickImportDeviceConfig() {
    var szIP = document.getElementById("DeviceIP").value, szFileName = $(
        "#configFile").val();
    ...
    if (iStatus == 0) {
        ...
        if (iProcess < 0) {
            ...
            if (0 == $("#restartDiv").length) {
                $("<div id='restartDiv' class='freeze'></div>").appendTo(
                    "body");
                var oSize = getWindowSize();
                $("#restartDiv").css({
                    width : oSize.width + "px",
...
function clickRemoteConfig() {
    var szIP = document.getElementById("DeviceIP").value, iDevicePort = parseInt(
            $("#deviceport").val(), 10)
        || "", szInfo = "";
...
function clickRestoreDefault() {
    var szIP = document.getElementById("DeviceIP").value, szMode = "basic";
...
function clickGetDeviceIP() {
    var iDeviceMode = parseInt($("#devicemode").val(), 10), szAddress = $(
        "#serveraddress").val(), iPort = parseInt($("#serverport")
            .val(), 10) || 0, szDeviceID = $("#deviceid").val(), szDeviceInfo = "";
...
function clickGoPreset() {
    ...
    if (oWndInfo != null) {
        WebVideoCtrl.I_GoPreset(iPresetID, {
            success : function(xmlDoc) {
                //alert(oWndInfo.szIP + " 调用预置点成功！");
            },
            error : function() {
                //alert(oWndInfo.szIP + " 调用预置点失败！");
            }
        });
    }
}
...
function clickSetPreset() {
    var p = $("#Preset").val();
    var bsid = $("#bsLocation").val().split(":")[0];
    var videoid = $("#hzVideoid").val();
    if (p == null || p == "") {
        //alert("预置位不能为空！");
        return false;
    }else if(p==0){
        ...
        if (bsid > 0) {
            var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex);
            var iPresetID = parseInt(p);
            if (oWndInfo != null) {
                WebVideoCtrl.I_SetPreset(iPresetID, {
                    success : function(xmlDoc) {
                        var obj = {
                            "presetPoint.videoId" : videoid,
                            "presetPoint.presetPoint" : p,
                            "presetPoint.bsLocationId" : bsid
                        };
                        $.post("video_designPresetPoint", obj, function(
                            date) {
                            //alert(" 设置预置点成功！");
                        });
